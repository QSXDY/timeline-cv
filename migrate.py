...
def copy_photo(cat_name: str, proj_title: str, rel: str) -> str:
    ...
    _img_seq[0] += 1
    name = "%03d_%s" % (_img_seq[0], os.path.basename(rel))
...
def main():
    ...
    # 1. admin
    cur.execute("INSERT INTO admin (id, username, password_hash) VALUES (1, ?, ?)",
                (ADMIN_USER, hash_password(ADMIN_PASS)))

    # 2. basic_info
    cur.execute("""INSERT INTO basic_info
        ...
        "视频运营", "12–18K", "昆明", "136810610@qq.com", "男 · 37 岁 · 从业 13 年",
    ))

    # 3. summary + cards
    cur.execute("INSERT INTO summary (id, title, subtitle) VALUES (1, ?, ?)", (
        "内容出身，技术落地，最近三年做AIGC。",
    ...
    for i, (t, c) in enumerate(cards):
        cur.execute("INSERT INTO summary_cards (title, content, sort) VALUES (?, ?, ?)", (t, c, i))

    # 4. categories + projects + images
    n_img = 0
    for ci, (gname, gcount, items, chips) in enumerate(GROUPS):
        cur.execute("INSERT INTO categories (name, subtitle, tags, kind, sort) VALUES (?, ?, ?, 'group', ?)",
                    (gname, gcount, ",".join(chips), ci))
        cid = cur.lastrowid
        for pi, (title, desc, photos) in enumerate(items):
            cur.execute("INSERT INTO projects (category_id, title, desc, detail_html, sort) VALUES (?, ?, ?, '', ?)",
                        (cid, title, desc, pi))
            pid = cur.lastrowid
            if photos:
                for prefix, count, label in photos:
                    for rel in folder_files(prefix)[:count]:
                        cur.execute(
                            ...
                            (pid, copy_photo(gname, title, rel),
                             "v" if portrait(os.path.join(FIG, rel)) else "h", label, n_img))
                        n_img += 1
    ...
    cur.execute("INSERT INTO categories (name, subtitle, tags, kind, sort) "
                "VALUES ('AIGC 经历 · 2023 至今', '把模型能力，变成团队和客户用得上的东西。', '', 'aigc', 99)")
    cid = cur.lastrowid
    cur.execute("INSERT INTO projects (category_id, title, desc, detail_html, sort) VALUES (?, ?, ?, ?, 0)", (
        cid, "AIGC 内容与应用运营",
    ...
    for i, (prefix, _count, label) in enumerate(AIGC_PHOTOS):
        ...
        cur.execute(
            "INSERT INTO images (project_id, filename, orientation, caption, sort) VALUES (?, ?, ?, ?, ?)",
            (pid, copy_photo("AIGC 经历 · 2023 至今", "AIGC 内容与应用运营", rel),
             "v" if portrait(os.path.join(FIG, rel)) else "h", label, i))
        n_img += 1

    # 6. experiences
    exps = [
        ("技术部总监", "新媒体公司（可见）", "内容全链路统筹 + 技术落地", "2016 — 2026", [
        ...
        ("视频部总监", "广告公司", "地产板块全案", "2015 — 2016", [
            "主导地产板块全案业务：广告视频制作、节目包装、平面物料设计、品牌活动策划。",
            "带领团队完成多档播出节目与户外广告项目，统筹策划并落地多项大型线下商业活动。",
        ]),
        ("摄像 / 剪辑 / 包装", "电视台", "大学期间寒暑假即在电台、电视台实习", "2013 — 2015", [
            "负责新闻、纪录片拍摄剪辑、节目包装、平面设计与线下活动策划。",
            "独立完成多部可上星播出电视节目的全流程制作。",
        ]),
    ]
    for i, (pos, org, org_desc, years, lines) in enumerate(exps):
        cur.execute("INSERT INTO experiences (position, org, org_desc, years, content, detail_html, sort) VALUES (?, ?, ?, ?, ?, '', ?)",
                    (pos, org, org_desc, years, "\n".join(lines), i))

    # 7. skills + cards + tags
    cur.execute("INSERT INTO skills (id, title, subtitle) VALUES (1, '会拍会剪，也懂技术。', '')")
    skill_data = [
        ("影视与运营", ["方案与脚本", "分镜", "摄像机 / 单反 / 微单", "Edius", "Premiere", "After Effects",
                      "Audition", "Photoshop", "Illustrator", "多机位直播推流", "新媒体代运营"]),
        ("技术与AIGC", ["大模型 API 聚合与中转", "模型选型与评测", "提示词工程", "智能体与工作流", "文生图 / 图生图",
                       "文生音乐 · Suno", "文生视频", "小程序开发", "网页与商城架构", "服务器与节点运维", "Office 办公"]),
    ]
    for i, (t, tags) in enumerate(skill_data):
        cur.execute("INSERT INTO skill_cards (title, sort) VALUES (?, ?)", (t, i))
        cid = cur.lastrowid
        for j, tag in enumerate(tags):
            cur.execute("INSERT INTO skill_tags (card_id, name, sort) VALUES (?, ?, ?)", (cid, tag, j))
